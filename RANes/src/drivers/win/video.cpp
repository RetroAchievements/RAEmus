...
{	
	...
	if( lpDDSAchievements != NULL )
	{
		...
		if( lpDDSAchievements->GetDC( &hDC )== DD_OK )
		{
			...
			//	(nIn&0x40)?'<':' ', (nIn&0x10)?'^':' ', (nIn&0x80)?'>':' ', (nIn&0x20)?'v':' ',
			//	(nIn&0x01)?'A':' ', (nIn&0x02)?'B':' ', (nIn&0x08)?'S':' ', (nIn&0x04)?'s':' ',
...
static void BlitScreenWindow(unsigned char *XBuf)
{
	...
	//if (eoptions & EO_BESTFIT && (bestfitRect.top || bestfitRect.left))	//	##SD TBD!
	if( 0 )
	{
		// blit with resizing
		//	I.e. causes that TV-overscan thing. Necessary!
		...
		RenderAchievementOverlays( srect );
		...
		if (bestfitRect.top)
		{
			...
			IDirectDrawSurface7_Blt(lpDDSPrimary, &blitRect, lpDDSAchievements, NULL, DDBLT_COLORFILL | DDBLT_ASYNC, &blitfx);
			// lower border
	...
	else
	{
		...
		if (IDirectDrawSurface7_Blt(lpDDSAchievements, NULL, lpDDSBack, &srect, DDBLT_ASYNC, 0) == DD_OK)
		{
			RenderAchievementOverlays( wrect );

			IDirectDrawSurface7_Blt(lpDDSPrimary, &wrect, lpDDSAchievements, NULL, DDBLT_WAIT | DDBLT_ASYNC, 0 );
			//if(IDirectDrawSurface7_Blt(lpDDSPrimary, &wrect, lpDDSAchievements, NULL, DDBLT_ASYNC, 0) != DD_OK)
			//{
			//	ddrval = IDirectDrawSurface7_Blt(lpDDSPrimary, &wrect, lpDDSAchievements, NULL, DDBLT_WAIT, 0);
			//	if(ddrval != DD_OK)
			//	{
			//		if(ddrval == DDERR_SURFACELOST)
			//		{
