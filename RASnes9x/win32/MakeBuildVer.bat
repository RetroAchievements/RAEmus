...
git describe --tags --match "RASnes9x.*" > Temp.txt
set /p ACTIVE_TAG=<Temp.txt
for /f "tokens=1,2 delims=-" %%a in ("%ACTIVE_TAG:~9%") do set VERSION_TAG=%%a&set VERSION_REVISION=%%b
...
@echo #define RASNES9X_VERSION "%VERSION_NUM%.%VERSION_REVISION%" > BuildVer2.h
