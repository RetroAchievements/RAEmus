...
BOOL VBA::OnIdle(LONG lCount)
{
  ...
  } else if( emulating )
  {
	  ...
	  else if( active && paused )
	  {
		  ...
		  //theApp.renderedFrames--;
		  Sleep(10);
		  return TRUE;	//	Non-zero means we will receive more processing time! http://msdn.microsoft.com/en-us/library/3e077sxt.aspx
	  }
	  else if( !active )
	  {
		  //##RA
...
void VBA::loadSettings()
{
  ...
  filterType = regQueryDwordValue("filter", 14);	//	##RA	0
