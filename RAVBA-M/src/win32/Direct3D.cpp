...
//	#RA
void RenderAchievementOverlays( const RECT& rc, LPDIRECT3DSURFACE9 pSurface )
{
	...
	if( pSurface != NULL )
	{
		...
		if( pSurface->GetDC( &hDC ) == S_OK )
		{
			systemReadJoypads();
			u32 nKeysDown = systemReadJoypad(-1 );

			ControllerInput input;
			input.m_bDownPressed		= (nKeysDown&128)!=0;
			input.m_bUpPressed			= (nKeysDown&64)!=0;
			input.m_bLeftPressed		= (nKeysDown&32)!=0;
			input.m_bRightPressed		= (nKeysDown&16)!=0;
			input.m_bConfirmPressed		= (nKeysDown&1)!=0;		//	A
			input.m_bCancelPressed		= (nKeysDown&2)!=0;		//	B
			input.m_bQuitPressed		= (nKeysDown&8)!=0;		//	Return

			RECT rcDest = rc;

			RA_UpdateRenderOverlay( hDC, &input, ((float)nDelta / 1000.0f), &rcDest, theApp.fullScreenStretch, theApp.paused );
...
void Direct3DDisplay::render()
{
	...
	//{
	//	RenderAchievementOverlays( target, pBackBuffer );
	//	pBackBuffer->Release();
	//	pBackBuffer = NULL;
	//}
