...
WPARAM CMainGui::ProcessAllMessages(void)
{
	...
	while (GetMessage(&msg, NULL, 0, 0))
	{
		...
		if (GetForegroundWindow() == m_hMainWindow && m_Menu->ProcessAccelerator(m_hMainWindow, &msg)) { continue; }
    ...
    case WM_COMMAND:
        {
            ...
            default:
                if (_this->m_Menu)
                {
                    ...
                    else if (GetForegroundWindow() == hWnd && _this->m_Menu->ProcessMessage(hWnd, HIWORD(wParam), LOWORD(wParam)))
                    {
                        return true;
                    }
                }
