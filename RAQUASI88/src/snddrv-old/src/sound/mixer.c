...
#if	1		/* QUASI88 */
	if (wavfile && mixer_sound_enabled)
		wav_add_data_16(wavfile, mix_buffer, samples_this_frame * ((is_stereo) ? 2 : 1));
#endif		/* QUASI88 */
...
int sound_wavfile_open(const char *filename)
{
	...
	wavfile = wav_open(filename, Machine->sample_rate, ((is_stereo) ? 2 : 1));

	if (wavfile) {

		wavfile_sample_rate = Machine->sample_rate;
		wavfile_is_stereo   = is_stereo;
		return TRUE;
	} else {
...
int sound_wavfile_damaged(void)
{
	...
	if (wavfile_sample_rate == Machine->sample_rate &&
		wavfile_is_stereo   == is_stereo)            return FALSE;
	else                                             return TRUE;
}
#endif
