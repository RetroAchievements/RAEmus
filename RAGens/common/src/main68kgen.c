...
static void getcondition(int cc) {
	...
	case 0x2:/* a */
		emit("test ah,41h\n");
		sprintf_s(optcc, 5, "z");
		sprintf_s(optrc, 5, "nz");
		break;
	case 0x3:/* be */
		emit("test ah,41h\n");
		sprintf_s(optcc, 5, "nz");
		sprintf_s(optrc, 5, "z");
		break;
	case 0x4:/* nc */
		emit("test ah,1\n");
	...
	case 0xA:/* ns */
		emit("or ah,ah\n");
		sprintf_s(optcc, 5, "ns");
		sprintf_s(optrc, 5, "s");
		break;
	case 0xB:/* s */
		emit("or ah,ah\n");
		sprintf_s(optcc, 5, "s");
		sprintf_s(optrc, 5, "ns");
		break;
	case 0xC:/* ge */
		getcondition_l_s_ns();
		sprintf_s(optcc, 5, "ns");
...
static void ea_step_precalc(int size, enum eamode mode, int reg) {
	char regs[100];
	if(reg == -1) sprintf_s(regs, 100, "ebx*4");
	else sprintf_s(regs, 100, "%d", reg * 4);
	switch(mode) {
	case dreg: case areg:
		break;
	case aind: case ainc: case adec:
...
static void flick_reg(char*op,int needxf,int affectx,int asl,int rotate){
	...
	if(main_ir==1){
		...
		** happen... */
		...
		sprintf_s(tmps, 5,"%d",quickvalue[main_reg]);
