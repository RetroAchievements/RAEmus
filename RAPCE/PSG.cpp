...
static BOOL			_bWaveCrash; //Kitao追加。DDA再生中にWaveデータが書き換えられたらTRUE
	...
	switch (reg & 15)
	{
		...
		case 4:	// ON, DDA, AL
			...
			if ((data & 0xC0) == 0x40)
			{
				...
				{
					for (i=0; i<32; i++)
						_Psg[_Channel].wave[i] = -14; //Waveデータを最小値で初期化
					_bWaveCrash = FALSE;
				}
			}
		...
		case 6:	// wave data  //Kitao更新。DDAモードのときもWaveデータを更新するようにした。v0.63。ファイヤープロレスリング
			...
			if (_Psg[_Channel].bDDA)
			{
				...
				if (!_Psg[_Channel].bOn) //DDAモード時にWaveデータを書き換えた場合
					_bWaveCrash = TRUE;
			}
			break;
...
psg_reset()
{
	...
	for (i=0; i<N_CHANNEL; i++)
		for (j=0; j<32; j++)
			_Psg[i].wave[j] = -14; //最小値で初期化。ファイプロ，フォーメーションサッカー'90，F1トリプルバトルで必要。
	for (j=0; j<32; j++)
		_Psg[3].wave[j] = 17; //ch3は最大値で初期化。F1トリプルバトル。v2.65
