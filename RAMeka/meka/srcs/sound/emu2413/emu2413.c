/***********************************************************************************
  ...
  2002 05-30 : Version 0.XX -- Fixed HH&CYM generator and all voice datas.
...
#endif
#ifdef PI
//#warning "PI was already defined"
#undef PI
#endif
...
makeSinTable (void)
{
  ...
  {
    fullsintable[i] = (e_uint16) lin2db (sin (2.0 * PI * (i+1) / PG_WIDTH));
  }
...
calc_slot_snare (OPLL_SLOT * slot, e_uint32 noise)
{
  if(slot->egout>=(DB_MUTE-1))
    ...
    return 0;
  else if(BIT(slot->pgout,7))
    return DB2LIN_TABLE[DB_POS(0)+slot->egout];
  else
    return DB2LIN_TABLE[DB_NEG(0)+slot->egout];
}
...
calc_slot_cym (OPLL_SLOT * slot, e_uint32 pgout_hh)
{
  ...
  if (slot->egout >= (DB_MUTE - 1))
    return 0;
      ...
      ((BIT(pgout_hh,PG_BITS-8)^BIT(pgout_hh,PG_BITS-1))|BIT(pgout_hh,PG_BITS-7)) ||  /* cited from fmopl.c in MAME */
      (BIT(slot->pgout,PG_BITS-7)&!BIT(slot->pgout,PG_BITS-5)) /* different from fmopl.c */
    )
    pgout = 0x30000 >> (18-PG_BITS);
  else
    pgout = 0x10000 >> (18-PG_BITS);

  return DB2LIN_TABLE[slot->sintbl[pgout&(PG_WIDTH-1)] + slot->egout];
}
...
calc_slot_hat (OPLL_SLOT *slot, e_int32 pgout_cym, e_uint32 noise)
{
  ...
  if (slot->egout >= (DB_MUTE - 1))
    return 0;
      ...
      ((BIT(slot->pgout,PG_BITS-8)^BIT(slot->pgout,PG_BITS-1))|BIT(slot->pgout,PG_BITS-7)) || /* cited from fmopl.c in MAME */
      (BIT(pgout_cym,PG_BITS-7)&!BIT(pgout_cym,PG_BITS-5)) /* different from fmopl.c */
    )
  {
    if(noise)
      pgout = 0/*(0x20000|(0xd000))*/ >> (18-PG_BITS);
    else
      pgout = (0x20000|(0xd000>>2)) >> (18-PG_BITS);
  }
  else
  {
    if(noise)
      pgout = 0/*(0xd000>>2)*/ >> (18-PG_BITS);
    else
      pgout = (0xd000) >> (18-PG_BITS);
  }
