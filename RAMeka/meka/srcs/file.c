...
int             Load_ROM_Main ()
{
	...
	if ( err == MEKA_ERR_OK )
	{
		if ( g_machine.driver_id == DRV_SMS )
			RAMeka_RA_MountROM( MasterSystem );

		else if ( g_machine.driver_id == DRV_GG )
			RAMeka_RA_MountROM( GameGear );

		else if ( g_machine.driver_id == DRV_COLECO )
			RAMeka_RA_MountROM( Colecovision );

		else if ( g_machine.driver_id == DRV_SG1000 )
			RAMeka_RA_MountROM( SG1000 );
	}
